...
export class TutoresFacade {
    private readonly subject = new BehaviorSubject<TutoresState>(initialState);
    readonly state$ = this.subject.asObservable();

    private seq = 0;
    private searchCache: SearchCache | null = null;

    getSnapshot(): TutoresState {
    ...
    async load(pageOverride?: number): Promise<void> {
        ...
        try {
            ...
            const result = await listTutores(req);
            ...
            const message =
                err instanceof ApiError ? err.message : err instanceof Error ? err.message : "Falha ao carregar tutores.";
    ...
    private applySearchPage(page: number): void {
        const curr = this.subject.getValue();
    ...
    private async buildSearchCache(qLower: string): Promise<void> {
        ...
        try {
            ...
            const first = await listTutores({ page: 0, size: fetchSize });
            if (requestId !== this.seq) return;

            const totalPages = Math.max(1, Math.ceil(first.total / fetchSize));

            const restPromises: Array<Promise<{ items: Tutor[]; total: number; page?: number; size?: number }>> = [];
            for (let p = 1; p < totalPages; p++) {
                restPromises.push(listTutores({ page: p, size: fetchSize }));
            }
            ...
            const filtered = all.filter((t) => t.nome.toLowerCase().includes(qLower));
            ...
            const message =
                err instanceof ApiError ? err.message : err instanceof Error ? err.message : "Falha ao buscar tutores."
