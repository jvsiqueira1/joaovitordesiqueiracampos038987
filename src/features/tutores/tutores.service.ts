...
function toTutor(dto: unknown): Tutor {
  ...
  const pets = Array.isArray(petsRaw)
    ? petsRaw.map(toLinkedPet).filter((p): p is LinkedPet => !!p)
    : undefined;

  if (!id || !nome || !telefone) {
    throw new ApiError("Resposta inválida: Tutor sem campos obrigatórios (id/nome/telefone).", {
      details: dto,
    });
  }
  ...
  const cpf =
    asString(dto["cpf"]) ??
    (typeof dto["cpf"] === "number" && Number.isFinite(dto["cpf"])
      ? String(dto["cpf"])
      : undefined);
...
function toLinkedPet(value: unknown): LinkedPet | undefined {
  ...
  const nome = asString(value["nome"]);
  const idade =
    typeof value["idade"] === "number" && Number.isFinite(value["idade"])
      ? value["idade"]
      : undefined;
